  ...
  <div id="guide">
    <div class="PCshow">
      <el-menu
        ...
        @select="handleSelect"
      >
        <span class="welcome">欢迎来到米奇妙妙污</span>
        ...
        <el-submenu index="/articleAll">
          ...
          <el-menu-item index="/articleAll">Lists</el-menu-item>

          <el-menu-item
            @click="something()"
            index
            v-bind:class="{ editable: getifeditable, uneditable: !getifeditable }"
          >Add article</el-menu-item>
        </el-submenu>

        <el-menu-item index="/message">LeaveMessage</el-menu-item>

        <el-submenu index>
          <template slot="title">Categories</template>

          <template v-for="(item,index) in catelist">
            <el-menu-item :key="index" index="/Categories/item" @click="ChangeCate(item)">{{item}}</el-menu-item>
          </template>
        </el-submenu>
        ...
        <el-menu-item index>
          <a
            href="https://www.zhihu.com/people/jin-yu-69-77-42/activities"
            style="text-decoration:none"
          >ZhiHu</a>
        </el-menu-item>
      </el-menu>
    </div>
    <li class="el-icon-arrow-right folder" v-on:click="ishow=ishow==true?false:true"></li>
      ...
      <div class="Phoneshow" v-if="gishow" :key="1">
        ...
        <el-menu class="el-menu-vertical-demo" router :collapse="isCollapse">
          <i
            ...
            style="cursor:pointer;z-index: 998;"
          ></i>
      ...
      <el-dialog title="请输入信息:" :visible.sync="dialogFormVisible" :key="2">
        <el-form>
          ...
          <el-form-item label="发表时间:" class="inline-form inline-form-right">
            <el-date-picker style="width: 70%;" type="date" placeholder="选择日期" v-model="form.date"></el-date-picker>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    com(ob1, ob2) {
      if (ob1.date > ob2.date) return -1;
      else if (ob1.date < ob2.date) return 1;
      else return 0;
    }
  },
  beforeCreate: function() {
    this.$ajax.get("/api/getArticles").then(ress => {
      var res;
      ...
      for (let item in res) this.catelist.push(res[item].cate);
  ...
  mounted() {
    ...
    if (confirm == "admin") this.iseditable = true;
